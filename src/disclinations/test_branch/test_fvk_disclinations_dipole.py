...
from disclinations.utils.la import (
    compute_cell_contributions,
    compute_disclination_loads,
)
...
# Point sources
disclination_points_list = [[-0.3, 0.0, 0], [0.3, 0.0, 0]]

if mesh.comm.rank == 0:
    # point = np.array([[0.68, 0.36, 0]], dtype=mesh.geometry.x.dtype)
    disclinations = [
        np.array([disclination_points_list[0]], dtype=mesh.geometry.x.dtype),
...
W_ext = Constant(mesh, np.array(0.0, dtype=PETSc.ScalarType)) * w * dx
...
b = compute_disclination_loads(
    disclinations, disclination_power_list, Q, V_sub_to_V_dofs=Q_v_to_Q_dofs, V_sub=Q_v
)
    ...
    if mesh.comm.rank == 0:
        for index, disc_coord in enumerate(disclination_points_list):
            ...
            energy += (
                ...
                * thickness
                * 0.5
                * disclination_power_list[index]
                * v_exact(disc_coord_apprx)
            )
        return energy
    else:
        return 0.0
...
exact_energy_dipole = mesh.comm.allreduce(
    compute_exact_energy_dipole(_v_exact), op=MPI.SUM
)
...
print(f"Computed total energy: {energy_terms['total']}")
...
_plt, data = plot_profile(
    ...
    None,
    subplot=(1, 2),
    lineproperties={"c": "k", "label": f"$Mxx(x)$"},
    fig=fig,
    subplotnumber=1,
)
...
_plt, data = plot_profile(
    ...
    None,
    subplot=(1, 2),
    lineproperties={"c": "k", "label": f"$Pxx(x)$"},
    fig=fig,
    subplotnumber=2,
)
