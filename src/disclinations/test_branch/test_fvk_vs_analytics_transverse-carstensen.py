...
import ufl
import yaml
from disclinations.models import NonlinearPlateFVK, NonlinearPlateFVK_carstensen
...
outdir = "output"
prefix = os.path.join(outdir, "plate_fvk_analytic_transverse-carstensen")
...
def exact_membrane_energy(v, w):
    ...
    return assemble_scalar(
        form(
            (
                ((1 + nu) / (2 * E * h)) * ufl.inner(hessian(v), hessian(v))
                - nu / (2 * E * h) * ufl.inner(laplacian(v), laplacian(v))
            )
            * ufl.dx
        )
    )
...
for energy_type, energy_function in zip(
    ["bending", "membrane", "coupling"],
    [exact_bending_energy, exact_membrane_energy, exact_coupling_energy],
):
