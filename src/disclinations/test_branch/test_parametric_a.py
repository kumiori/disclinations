...
def run_experiment(FEMmodel, costantData, parameters, param):
    ...
    def nondim_transverse_load(x): return 1.0 + 0.0*x[0] + 0.0*x[1]
...
if __name__ == "__main__":
    ...
    a_list = list(np.linspace(1, 10, NUM_RUNS))
    a_range = []

    p_range = [1/el for el in a_list] # CFe: thickness values
    ...
    _experimental_data = []
    outdir = "output"
    prefix = os.path.join(outdir, "test_parametric_a")
