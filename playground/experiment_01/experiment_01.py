...
def run_experiment(mesh: None, parameters: dict, series: str):
    ...
    b = compute_disclination_loads(
        points, signs, Q, V_sub_to_V_dofs=Q_v_to_Q_dofs, V_sub=Q_v
    )

    # Transverse load (analytical solution)
    def transverse_load(x):
        _f = (40 / 3) * (1 - x[0] ** 2 - x[1] ** 2) ** 4 + (16 / 3) * (
            11 + x[0] ** 2 + x[1] ** 2
        )
        return _f * f_scale
    ...
    W_ext = dolfinx.fem.Constant(mesh, 0.0) * f * w * dx
    print(b_scale)
    ...
    with dolfinx.common.Timer(f"~Postprocessing and Vis") as timer:
        ...
        data["v_L2"] = comm.allreduce(
            dolfinx.fem.assemble_scalar(dolfinx.fem.form(ufl.inner(v, v) * dx)),
            op=MPI.SUM,
        )
        data["w_L2"] = comm.allreduce(
            dolfinx.fem.assemble_scalar(dolfinx.fem.form(ufl.inner(w, w) * dx)),
            op=MPI.SUM,
        )
...
if __name__ == "__main__":
    ...
    parameters, signature = load_parameters(
        "../../src/disclinations/test_branch/parameters.yml"
    )
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(
        experimental_data["thickness"], experimental_data["w_L2"], label=r"$w_{L^2}$"
    )
    plt.plot(
        experimental_data["thickness"], experimental_data["v_L2"], label=r"$v_{L^2}$"
    )
    plt.xlabel("Thickness")
    plt.ylabel("L2 Norms")
