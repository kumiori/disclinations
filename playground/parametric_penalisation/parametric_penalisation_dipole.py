...
def load_parameters(filename):
    ...
    params["model"]["E"] = 1
    params["model"]["α_adim"] = 10
    params["model"]["thickness"] = 1 / params["model"]["α_adim"]
    params["model"]["γ_adim"] = 0.0
...
if __name__ == "__main__":
    ...
    with dolfinx.common.Timer(f"~Computation Experiment") as timer:
        for i, a in enumerate(np.logspace(np.log10(10), np.log10(1000), num=num_runs)):
            # parameters["model"]["alpha_penalty"] = a

            if changed := update_parameters(parameters, "alpha_penalty", float(a)):
